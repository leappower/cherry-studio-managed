...
- [ ] **A0 环境**：服务端部署在哪台机器？（本机 / 192.168.3.181 / 云服务器）
- [ ] **A0 端口**：服务端 WS 端口定多少？（建议 23334，与 23333 呼应）
- [ ] **A1 鉴权**：设备注册 token 方案（简单 token / 每设备独立 key）
- [ ] **A3 数据范围**：采集哪些数据？（会话数/消息数/报错，还是含内容？需老板确认合规边界）
...
1. **服务端部署位置**：本机 / 181 / 云服务器？
2. **服务端 WS 端口**：23334 可以吗？
3. **设备鉴权强度**：简单 token 够，还是要每设备独立 key？
4. **数据采集范围**：只采使用统计（会话/消息数），还是含内容？（合规边界）
5. **是否先做 A0-A1**（服务端骨架 + 单机派发闭环）验证，再扩展 A2-A4？
